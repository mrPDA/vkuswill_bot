...
Langfuse — система LLM-observability: трейсы взаимодействий бота с **OpenAI-compatible** провайдером (production runtime: **Yandex Cloud AI Studio / Qwen** по `LLM_MODEL` и `LLM_PROVIDER=qwen_openai`; см. [архитектуру](ARCHITECTURE.md)). Каждое сообщение пользователя создаёт **trace**, каждый вызов LLM — **generation**, каждый вызов инструмента — **span**.
