...
_CLASSIFY_PROMPT_STUB = (
    ...
    "4. Если перечислены товары/продукты для покупки без блюда — cart.\n"
    "5. 'на N человек' БЕЗ слов меню/рацион/план — это НЕ meal_plan."
    " Мероприятия (день рождения, шашлык, пикник) = cart или recipe.\n"
    "6. Запросы с диетой (кето, веган) + конкретный приём пищи (завтрак)"
    " = recipe, если НЕ просят генерировать план на период.\n"
    "7. confidence должен быть числом от 0 до 1.\n"
    "8. reason должен быть коротким, не более 12 слов.\n\n"
    "Примеры:\n"
