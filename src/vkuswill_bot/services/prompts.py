...
_FALLBACK_PROFILES: dict[str, str] = {
    ...
    "ВАЖНО: если пользователь назвал конкретные товары или продукты,"
    " сразу ищи их и собирай корзину. Не задавай уточняющих вопросов"
    " на чёткие списки. Уточняй только если запрос действительно неоднозначен.\n"
    ...
    " '3 шт яблок' = 3 штуки, '5 шт яиц' = 5 штук.\n"
    ...
    "ГОТОВЫЕ БЛЮДА: НЕ подставляй готовые блюда (суп, каша, салат)"
    " вместо сырых ингредиентов. Если нужный ингредиент не найден —"
    " добавь в список ненайденных, а не бери готовое блюдо.\n"
    ...
    " пользователя на большой объём.\n"
    "НЕСКОЛЬКО БЛЮД: если пользователь просит собрать продукты для"
    " нескольких блюд (завтрак+обед+ужин), обработай ВСЕ блюда."
    " Для каждого вызови recipe_ingredients или найди ингредиенты."
    " НЕ останавливайся после первого блюда.",
