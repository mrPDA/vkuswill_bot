...
Передавай query НА РУССКОМ (борщ, куриная грудка, плов).
...
а) Сделай 1-2 поиска через vkusvill_products_search — ЗАПОМНИ xml_id! \
б) Из результатов выбери 5-6 ЛУЧШИХ кандидатов (самые подходящие). \
в) Проверь КБЖУ через nutrition_lookup ТОЛЬКО для этих 5-6 кандидатов. \
НЕ проверяй ВСЕ 10 результатов — это потратит слишком много шагов! \
г) Отфильтруй подходящие (ккал ≤ лимита) — используй xml_id из шага (а). \
д) СРАЗУ вызови vkusvill_cart_link_create с этими xml_id. \
НЕ ищи товары повторно! НЕ делай больше поисков! Создай корзину!
