...
class DialogManager:
    ...
    - Async (aget_history, save_history, trim_list, areset) — единый интерфейс
      с RedisDialogManager, используется в chat engine.
    """
    ...
    def __init__(
        ...
        self._conversations: OrderedDict[int, list[dict[str, object]]] = OrderedDict()
    ...
    def get_history(self, user_id: int) -> list[dict[str, object]]:
        ...
        if user_id in self._conversations:
            ...
            self._conversations[user_id] = [{"role": "system", "content": get_system_prompt()}]
    ...
    async def aget_history(self, user_id: int) -> list[dict[str, object]]:
    ...
    def trim_list(self, history: list[dict[str, object]]) -> list[dict[str, object]]:
    ...
    @property
    def conversations(self) -> OrderedDict[int, list[dict[str, object]]]:
