...
class TestHardcodedSecrets:
    ...
    @pytest.mark.parametrize("path,code", _all_source_code(), ids=lambda p: str(p) if isinstance(p, Path) else "code")
        ...
        for line in content.splitlines():
            ...
            if "=" in line:
                ...
                assert len(value) < 50 or value.startswith("your_"), (
                    f".env.example: подозрительное значение для {key.strip()}: "
                    f"{value[:30]}..."
                )
...
DANGEROUS_FUNCTIONS = [
    ...
    ("subprocess shell=True", r'subprocess\.\w+\s*\([^)]*shell\s*=\s*True',
...
class TestDangerousFunctions:
    ...
    def test_no_dangerous_functions(self, path: Path, code: str):
        ...
        violations = []
        for name, pattern, description in DANGEROUS_FUNCTIONS:
        ...
        for path, code in _all_source_code():
            ...
            for pattern, description in ssl_disabled_patterns:
                ...
                for match in matches:
                    ...
                    findings.append(
                        f"  {rel_path}:{line_num}: SSL отключён — {description}"
                    )
...
class TestInformationLeakage:
    ...
    def test_error_messages_are_generic(self):
        ...
        error_responses = re.findall(
            r'response\s*=\s*\(\s*"([^"]*)"', handlers_code
        )

        for response in error_responses:
            # Не должно содержать технических деталей
            assert "traceback" not in response.lower(), (
                f"Ответ содержит traceback: {response[:50]}"
            )
            assert "exception" not in response.lower(), (
                f"Ответ содержит exception: {response[:50]}"
            )
            assert "stack" not in response.lower(), (
                f"Ответ содержит stack trace: {response[:50]}"
            )
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ExceptHandler):
                if node.type is None:
                    ...
                    violations.append(
                        f"  Строка {node.lineno}: bare except (без типа исключения)"
                    )
...
class TestGitignore:
    ...
    def test_env_in_gitignore(self):
        ...
        assert any(
            p.strip() in (".env", ".env*", ".env.*") for p in patterns
    ...
    def test_log_files_in_gitignore(self):
        ...
        assert any(
            "log" in p.strip().lower() for p in patterns
